...
#define GEOM GL_GEOMETRY_SHADER
...
u32 gfProgramPipelineCreate(u32 vert_program, u32 frag_program, u32 geom_shader);
  ...
  u32 geom_program)
{
  ...
  if(geom_program) glUseProgramStages(ppo, GL_GEOMETRY_SHADER_BIT, geom_program);
  ...
  const gpu_cmd_t * gpu_cmd)
{
  ...
  for(u32 i = 0; i < gpu_cmd_count; ++i)
  {
    ...
    // glPointSize(3.0f);
    glDrawArraysInstancedBaseInstance(GL_POINTS, (i32)cmd.first, (i32)cmd.count, (i32)cmd.instance_count, cmd.instance_first);
  }

...
#include <sstream>
#include <algorithm>
#include <boost/crc.hpp>        // for pdalboost::crc_optimal
...
class NodeImpl : public pdalboost::enable_shared_from_this<NodeImpl> {
public:
    virtual NodeType        type() = 0;
    void                    checkImageFileOpen(const char* srcFileName, int srcLineNumber, const char* srcFunctionName);
    virtual bool            isTypeEquivalent(pdalboost::shared_ptr<NodeImpl> ni) = 0;
    bool                    isRoot();
    pdalboost::shared_ptr<NodeImpl> parent();
    ustring                 pathName();
    ustring                 relativePathName(pdalboost::shared_ptr<NodeImpl> origin, ustring childPathName = ustring());
    ustring                 elementName();
    pdalboost::shared_ptr<ImageFileImpl> destImageFile();
    ...
    void                    setParent(pdalboost::shared_ptr<NodeImpl> parent, const ustring& elementName);
    bool                    isTypeConstrained();

    virtual pdalboost::shared_ptr<NodeImpl> get(const ustring& pathName);
    virtual void            set(const ustring& pathName, pdalboost::shared_ptr<NodeImpl> ni, bool autoPathCreate = false);
    virtual void            set(const std::vector<ustring>& fields, unsigned level, pdalboost::shared_ptr<NodeImpl> ni, bool autoPathCreate = false);

    virtual void            checkLeavesInSet(const std::set<ustring>& pathNames, pdalboost::shared_ptr<NodeImpl> origin) = 0;
    void                    checkBuffers(const std::vector<SourceDestBuffer>& sdbufs, bool allowMissing);
    bool                    findTerminalPosition(pdalboost::shared_ptr<NodeImpl> ni, uint64_t& countFromLeft);

    virtual void            writeXml(pdalboost::shared_ptr<ImageFileImpl> imf, CheckedFile& cf, int indent, const char* forcedFieldName=NULL) = 0;
...
protected: //=================
    ...
    friend class Encoder; //???
                                         ...
                                         NodeImpl(pdalboost::weak_ptr<ImageFileImpl> destImageFile);
    NodeImpl&                            operator=(NodeImpl& n);
    virtual pdalboost::shared_ptr<NodeImpl>  lookup(const ustring& /*pathName*/) {return(pdalboost::shared_ptr<NodeImpl>());}; //???
    pdalboost::shared_ptr<NodeImpl>          getRoot();

    pdalboost::weak_ptr<ImageFileImpl>       destImageFile_;
    pdalboost::weak_ptr<NodeImpl>            parent_;
...
public:
                        StructureNodeImpl(pdalboost::weak_ptr<ImageFileImpl> destImageFile);
    virtual             ~StructureNodeImpl() {};

    virtual NodeType    type();
    virtual bool        isTypeEquivalent(pdalboost::shared_ptr<NodeImpl> ni);
    virtual bool        isDefined(const ustring& pathName);
    virtual void        setAttachedRecursive();

    virtual int64_t     childCount();
    virtual pdalboost::shared_ptr<NodeImpl> get(int64_t index);
    virtual pdalboost::shared_ptr<NodeImpl> get(const ustring& pathName);
    virtual void        set(int64_t index, pdalboost::shared_ptr<NodeImpl> ni);
    virtual void        set(const ustring& pathName, pdalboost::shared_ptr<NodeImpl> ni, bool autoPathCreate = false);
    virtual void        set(const std::vector<ustring>& fields, unsigned level, pdalboost::shared_ptr<NodeImpl> ni, bool autoPathCreate = false);
    virtual void        append(pdalboost::shared_ptr<NodeImpl> ni);

    virtual void        checkLeavesInSet(const std::set<ustring>& pathNames, pdalboost::shared_ptr<NodeImpl> origin);

    virtual void        writeXml(pdalboost::shared_ptr<ImageFileImpl> imf, CheckedFile& cf, int indent, const char* forcedFieldName=NULL);
...
protected: //=================
    ...
    virtual pdalboost::shared_ptr<NodeImpl> lookup(const ustring& pathName);

    std::vector<pdalboost::shared_ptr<NodeImpl> > children_;
};

class VectorNodeImpl : public StructureNodeImpl {
public:
    explicit            VectorNodeImpl(pdalboost::weak_ptr<ImageFileImpl> destImageFile, bool allowHeteroChildren);
    ...
    virtual void        set(int64_t index, pdalboost::shared_ptr<NodeImpl> ni);
    //???virtual void   set(const ustring& pathName, pdalboost::shared_ptr<NodeImpl> ni);
    //???virtual void   append(pdalboost::shared_ptr<NodeImpl> ni);
...
class SourceDestBufferImpl : public pdalboost::enable_shared_from_this<SourceDestBufferImpl> {
public:
    SourceDestBufferImpl(pdalboost::weak_ptr<ImageFileImpl> destImageFile, const ustring pathName, int8_t* b,   const size_t capacity, bool doConversion = false,
                         bool doScaling = false, size_t stride = sizeof(int8_t));
    SourceDestBufferImpl(pdalboost::weak_ptr<ImageFileImpl> destImageFile, const ustring pathName, uint8_t* b,  const size_t capacity, bool doConversion = false,
                         bool doScaling = false, size_t stride = sizeof(uint8_t));
    SourceDestBufferImpl(pdalboost::weak_ptr<ImageFileImpl> destImageFile, const ustring pathName, int16_t* b,  const size_t capacity, bool doConversion = false,
                         bool doScaling = false, size_t stride = sizeof(int16_t));
    SourceDestBufferImpl(pdalboost::weak_ptr<ImageFileImpl> destImageFile, const ustring pathName, uint16_t* b, const size_t capacity, bool doConversion = false,
                         bool doScaling = false, size_t stride = sizeof(uint16_t));
    SourceDestBufferImpl(pdalboost::weak_ptr<ImageFileImpl> destImageFile, const ustring pathName, int32_t* b,  const size_t capacity, bool doConversion = false,
                         bool doScaling = false, size_t stride = sizeof(int32_t));
    SourceDestBufferImpl(pdalboost::weak_ptr<ImageFileImpl> destImageFile, const ustring pathName, uint32_t* b, const size_t capacity, bool doConversion = false,
                         bool doScaling = false, size_t stride = sizeof(uint32_t));
    SourceDestBufferImpl(pdalboost::weak_ptr<ImageFileImpl> destImageFile, const ustring pathName, int64_t* b,  const size_t capacity, bool doConversion = false,
                         bool doScaling = false, size_t stride = sizeof(int64_t));
    SourceDestBufferImpl(pdalboost::weak_ptr<ImageFileImpl> destImageFile, const ustring pathName, bool* b,     const size_t capacity, bool doConversion = false,
                         bool doScaling = false, size_t stride = sizeof(bool));
    SourceDestBufferImpl(pdalboost::weak_ptr<ImageFileImpl> destImageFile, const ustring pathName, float* b,    const size_t capacity, bool doConversion = false,
                         bool doScaling = false, size_t stride = sizeof(float));
    SourceDestBufferImpl(pdalboost::weak_ptr<ImageFileImpl> destImageFile, const ustring pathName, double* b,   const size_t capacity, bool doConversion = false,
                         bool doScaling = false, size_t stride = sizeof(double));
    SourceDestBufferImpl(pdalboost::weak_ptr<ImageFileImpl> destImageFile, const ustring pathName, std::vector<ustring>* b);
    ...
    void            checkCompatible(pdalboost::shared_ptr<SourceDestBufferImpl> newBuf);
...
public:
                        CompressedVectorNodeImpl(pdalboost::weak_ptr<ImageFileImpl> destImageFile);
    ...
    void                setPrototype(pdalboost::shared_ptr<NodeImpl> prototype);
    pdalboost::shared_ptr<NodeImpl> getPrototype();
    void                setCodecs(pdalboost::shared_ptr<VectorNodeImpl> codecs);
    pdalboost::shared_ptr<VectorNodeImpl> getCodecs();
    ...
    pdalboost::shared_ptr<CompressedVectorWriterImpl> writer(std::vector<SourceDestBuffer> sbufs);
    pdalboost::shared_ptr<CompressedVectorReaderImpl> reader(std::vector<SourceDestBuffer> dbufs);
...
protected: //=================
    ...
    pdalboost::shared_ptr<NodeImpl> prototype_;
    pdalboost::shared_ptr<VectorNodeImpl> codecs_;
...
public:
                        IntegerNodeImpl(pdalboost::weak_ptr<ImageFileImpl> destImageFile, int64_t value = 0, int64_t minimum = 0, int64_t maximum = 0);
...
public:
                        ScaledIntegerNodeImpl(pdalboost::weak_ptr<ImageFileImpl> destImageFile,
							int64_t value = 0, int64_t minimum = 0, int64_t maximum = 0,
                            double scale = 1.0, double offset = 0.0);

						ScaledIntegerNodeImpl(pdalboost::weak_ptr<ImageFileImpl> destImageFile,		//Added by SC
...
public:
                        FloatNodeImpl(pdalboost::weak_ptr<ImageFileImpl> destImageFile,
...
public:
    explicit            StringNodeImpl(pdalboost::weak_ptr<ImageFileImpl> destImageFile, const ustring value = "");
...
public:
                        BlobNodeImpl(pdalboost::weak_ptr<ImageFileImpl> destImageFile, int64_t byteCount);
                        BlobNodeImpl(pdalboost::weak_ptr<ImageFileImpl> destImageFile, int64_t fileOffset, int64_t length);
...
class ImageFileImpl : public pdalboost::enable_shared_from_this<ImageFileImpl> {
public:
					ImageFileImpl();
	void			construct2(const ustring& fileName, const ustring& mode, const ustring& configuration);
    pdalboost::shared_ptr<StructureNodeImpl> root();
    void            close();
    void            cancel();
    bool            isOpen();
    bool            isWriter();
...
protected: //=================
    ...
    pdalboost::shared_ptr<StructureNodeImpl> root_;
};
...
struct DecodeChannel {
    ...
    pdalboost::shared_ptr<Decoder> decoder;
    ...
    bool                inputFinished;
                        ...
                        DecodeChannel(SourceDestBuffer dbuf_arg, pdalboost::shared_ptr<Decoder> decoder_arg, unsigned bytestreamNumber_arg, uint64_t maxRecordCount_arg);
                        ~DecodeChannel();
...
public:
                CompressedVectorReaderImpl(pdalboost::shared_ptr<CompressedVectorNodeImpl> ni, std::vector<SourceDestBuffer>& dbufs);
                ~CompressedVectorReaderImpl();
    unsigned    read();
    unsigned    read(std::vector<SourceDestBuffer>& dbufs);
    void        seek(uint64_t recordNumber);
    bool        isOpen();
    pdalboost::shared_ptr<CompressedVectorNodeImpl> compressedVectorNode();
    void        close();
...
protected: //=================
    ...
    pdalboost::shared_ptr<CompressedVectorNodeImpl> cVector_;
    pdalboost::shared_ptr<NodeImpl>                 proto_;
...
public:
                CompressedVectorWriterImpl(pdalboost::shared_ptr<CompressedVectorNodeImpl> ni, std::vector<SourceDestBuffer>& sbufs);
...
protected: //=================
    ...
    std::vector<pdalboost::shared_ptr<Encoder> >  bytestreams_;
...
class Encoder {
public:
    static pdalboost::shared_ptr<Encoder>  EncoderFactory(unsigned bytestreamNumber,
                                       pdalboost::shared_ptr<CompressedVectorNodeImpl> cVector,
...
protected: //================
    ...
    pdalboost::shared_ptr<SourceDestBufferImpl>  sourceBuffer_;
...
class Decoder {
public:
    static pdalboost::shared_ptr<Decoder>  DecoderFactory(unsigned bytestreamNumber,
                                       pdalboost::shared_ptr<CompressedVectorNodeImpl> cVector,
...
protected: //================
    ...
    pdalboost::shared_ptr<SourceDestBufferImpl> destBuffer_;
...
struct IndexPacket {  /// Note this is whole packet, not just header
    ...
    pdalboost::uint8_t     packetType;     // = E57_INDEX_PACKET
    pdalboost::uint8_t     packetFlags;    // flag bitfields
    pdalboost::uint16_t    packetLogicalLengthMinus1;
    pdalboost::uint16_t    entryCount;
    pdalboost::uint8_t     indexLevel;
    pdalboost::uint8_t     reserved1[9];   // must be zero
    struct IndexPacketEntry {
        pdalboost::uint64_t    chunkRecordNumber;
        pdalboost::uint64_t    chunkPhysicalOffset;
    } entries[MAX_ENTRIES];

                IndexPacket();
    void        verify(unsigned bufferLength=0, pdalboost::uint64_t totalRecordCount=0, pdalboost::uint64_t fileSize=0);
#ifdef E57_BIGENDIAN
    void        swab(bool toLittleEndian);
#else
    void        swab(bool /*toLittleEndian*/) {};
...
{
    bool                    isOpen_;
    pdalboost::shared_ptr<ImageFileImpl> imf_;
